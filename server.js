...
io.on("connection", (socket) => {
  ...
  socket.on("creator:switch", ({ mimeType } = {}, ack) => {
    ...
    socket
      .to(`stream:${streamId}`)
      .emit("stream:reset", { mimeType: stream.mimeType });
    ack && ack({ ok: true });
  ...
  socket.on("viewer:join", ({ streamId }, ack) => {
    ...
    io.to(stream.creatorSocketId).emit("creator:viewer-count", {
    ...
    console.log(
      `[viewer:join] ${username} → ${streamId} (total ${stream.viewers})`,
    );
  });
  ...
  socket.on("viewer:resync", ({ streamId }, ack) => {
    const stream = streams.get(streamId);
    if (!stream) return ack && ack({ error: "Stream not found" });
    ack &&
      ack({
